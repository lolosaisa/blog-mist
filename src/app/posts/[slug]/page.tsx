...
import { fetchMarkdown, getAllPostSlugs } from "@/lib/github";
...
export default async function PostPage({ params }: Params) {
  ...
  // 2️⃣ Fetch all posts for sidebar (same repo, same source)
  ...
  const recommendedPosts = posts
    .filter((p) => p.frontmatter?.title && p.frontmatter?.slug !== slug)
    .slice(0, 3);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 py-10">
          ...
          <aside className="hidden lg:block">
            ...
            {recommendedPosts.map((post) => (
              <PostPreview
                title={""} coverImage={""} date={""} excerpt={""} author={{
                  name: "",
                  picture: ""
                }} slug={""} key={post.frontmatter.title}
                {...post.frontmatter}              />
            ))}
          </aside>
        </div>
...
export async function generateMetadata(
  ...
  const markdown = await fetchMarkdown(params.slug);
  const { frontmatter } = await parseMarkdown(markdown);

  return {
    title: `${frontmatter.title} | Mist.cash Blog`,
    openGraph: {
